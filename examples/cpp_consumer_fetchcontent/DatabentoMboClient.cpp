...
DatabentoMboClient::DatabentoMboClient(uint64_t client_id, const std::string& name, std::shared_ptr<OrderBook> order_book,
                                     uint64_t tracked_user_id, uint64_t slippage_delay_ns)
    : order_book_(order_book), client_id_(client_id), client_name_(name), tracked_user_id_(tracked_user_id) {
    portfolio_manager_ = std::make_shared<PortfolioManager>("portfolio_" + std::to_string(tracked_user_id) + ".csv");
    ...
    tob_tracker_ = std::make_shared<TopOfBookTracker>(default_symbol, current_date + "_" + current_date);
    
    std::cout << "[DatabentoMboClient] Initialized with order ID tracking for user " << tracked_user_id << std::endl;
}

// ========== IClient Interface Implementation ==========

uint64_t DatabentoMboClient::SubmitOrder(uint64_t user_id, bool is_buy, uint64_t quantity, uint64_t price, 
...
uint64_t DatabentoMboClient::SubmitOrder(uint64_t user_id, bool is_buy, uint64_t quantity, uint64_t price) {
    ...
    try {
        // Notify portfolio manager about the order submission
        if (portfolio_manager_) {
            portfolio_manager_->OnOrderSubmitted(order_id, user_id, is_buy, quantity, price, 0);
        }
...
bool DatabentoMboClient::ModifyOrder(uint64_t order_id, uint64_t new_quantity, uint64_t new_price) {
    if (!running_ || !order_book_) {
        return false;
    }
...
void DatabentoMboClient::OnTradeExecuted(const Trade& trade) {
    std::cout << "[CLIENT-TRADE] Aggressor=" << trade.aggressor_user_id << " (Order " << trade.aggressor_order_id 
    ...
    // Notify portfolio manager about the trade (it will only track if orders are being monitored)
    if (portfolio_manager_) {
...
void DatabentoMboClient::OnOrderModified(uint64_t order_id, uint64_t new_quantity, uint64_t new_price) {
    (void)order_id; (void)new_quantity; (void)new_price; // Suppress unused parameter warnings
}

void DatabentoMboClient::OnOrderRejected(uint64_t order_id, const std::string& reason) {
...
void DatabentoMboClient::OnTopOfBookUpdate(uint64_t best_bid, uint64_t best_ask, uint64_t bid_volume, uint64_t ask_volume) {
    ...
    if (tob_tracker_ && tob_tracker_->IsCSVEnabled()) {
        ...
        // Use current timestamp and symbol
...
void DatabentoMboClient::Initialize() {
    ...
    if (portfolio_manager_) {
        std::cout << "[CLIENT] Tracking user " << tracked_user_id_ << " in portfolio" << std::endl;
    }
...
uint64_t DatabentoMboClient::GetClientId() const {
    return client_id_;
}

std::string DatabentoMboClient::GetClientName() const {
    return client_name_;
}

// Portfolio management access
std::shared_ptr<PortfolioManager> DatabentoMboClient::GetPortfolioManager() const {
    return portfolio_manager_;
}

std::shared_ptr<TopOfBookTracker> DatabentoMboClient::GetTopOfBookTracker() const {
    return tob_tracker_;
}
...
bool DatabentoMboClient::IsUserTracked(uint64_t user_id) const {
    if (portfolio_manager_) {
        return portfolio_manager_->IsUserTracked(user_id);
    }
    return false;
}
...
KeepGoing DatabentoMboClient::ProcessMboMessage(const MboMsg& mbo) {
    ...
    switch (mbo.action) {
        case Action::Add: {
            ...
            // Convert Databento UnixNanos timestamps to uint64_t (nanoseconds since Unix epoch)
            // Use .time_since_epoch().count() to extract the underlying value
            uint64_t ts_received = static_cast<uint64_t>(mbo.ts_recv.time_since_epoch().count());
            // Calculate execution time: received time + in_delta (time from reception to processing)
            uint64_t ts_executed = ts_received + static_cast<uint64_t>(mbo.ts_in_delta.count());

            // Debug: Print timestamp info for first few orders to verify conversion
            static int ts_debug_count = 0;
            if (ts_debug_count < 5) {
                std::cout << "[TIMESTAMP-DEBUG] Order " << order_id << " - ts_received: " << ts_received 
                          << " ns, ts_executed: " << ts_executed << " ns, delta: " << mbo.ts_in_delta.count() << " ns" << std::endl;
                ts_debug_count++;
            }
            ...
            try {
                order_book_->AddOrder(order_id, 1, is_buy, size, price_for_orderbook, ts_received, ts_executed);
