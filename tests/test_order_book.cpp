...
TEST_F(OrderBookTest, AddSingleBuyOrder) {
    book->AddOrder(1, true, 100, 10000, 0, 0);  // Buy 100 @ 100.00
...
TEST_F(OrderBookTest, AddSingleSellOrder) {
    book->AddOrder(2, false, 150, 10050, 0, 0);  // Sell 150 @ 100.50
...
TEST_F(OrderBookTest, MultipleBuyOrdersOrdering) {
    book->AddOrder(1, true, 100, 10000, 0, 0);  // Buy 100 @ 100.00
    book->AddOrder(1, true, 150, 10020, 0, 0);  // Buy 150 @ 100.20
    book->AddOrder(1, true, 200, 9980, 0, 0);   // Buy 200 @ 99.80
...
TEST_F(OrderBookTest, MultipleSellOrdersOrdering) {
    book->AddOrder(2, false, 100, 10050, 0, 0);  // Sell 100 @ 100.50
    book->AddOrder(2, false, 150, 10030, 0, 0);  // Sell 150 @ 100.30
    book->AddOrder(2, false, 200, 10070, 0, 0);  // Sell 200 @ 100.70
...
TEST_F(OrderBookTest, OrderCancellation) {
    // Add orders
    uint64_t order_id_1 = book->AddOrder(1, true, 100, 10000, 0, 0);
    book->AddOrder(1, true, 150, 10020, 0, 0);
    uint64_t order_id_3 = book->AddOrder(2, false, 200, 10050, 0, 0);
...
TEST_F(OrderBookTest, DuplicateOrderId) {
    book->AddOrder(1, true, 100, 10000, 0, 0);
    // This test is no longer valid as the OrderBook doesn't see external IDs.
    // The check is now in the DatabentoProcessor.
    // We can't easily test this here without a mock client.
    // For now, we'll just add another order to ensure it doesn't crash.
    book->AddOrder(2, false, 150, 10050, 0, 0);
    SUCCEED();
}
...
TEST_F(OrderBookTest, BasicMatchingBuyCrossesSpread) {
    ...
    book->AddOrder(1, true, 80, 10050, 0, 0);  // Buy 80 @ 100.50 (matches)
...
TEST_F(OrderBookTest, BasicMatchingSellCrossesSpread) {
    ...
    book->AddOrder(2, false, 80, 10000, 0, 0);  // Sell 80 @ 100.00 (matches)
...
TEST_F(OrderBookTest, PartialFillMultipleLevels) {
    ...
    book->AddOrder(2, false, 50, 10050, 0, 0);   // Sell 50 @ 100.50
    book->AddOrder(2, false, 75, 10060, 0, 0);   // Sell 75 @ 100.60
    book->AddOrder(2, false, 100, 10070, 0, 0);  // Sell 100 @ 100.70

    EXPECT_EQ(book->GetTotalAskVolume(), 225);
    
    // Add large buy order that should match multiple levels
    book->AddOrder(1, true, 200, 10065, 0, 0);  // Buy 200 @ 100.65
...
TEST_F(OrderBookTest, TimePriorityAtSamePrice) {
    ...
    book->AddOrder(1, true, 100, 10000, 0, 0);  // First order
    book->AddOrder(2, true, 150, 10000, 0, 0);  // Second order
    book->AddOrder(3, true, 200, 10000, 0, 0);  // Third order

    EXPECT_EQ(book->GetTotalBidVolume(), 450);
    EXPECT_EQ(book->GetBestBid(), 10000);
    
    // Add sell order that partially fills
    book->AddOrder(4, false, 250, 10000, 0, 0);  // Sell 250 @ 100.00
...
TEST_F(OrderBookTest, EmptyAfterFullMatching) {
    ...
    book->AddOrder(1, true, 100, 10000, 0, 0);   // Buy 100 @ 100.00
    book->AddOrder(2, false, 100, 10000, 0, 0);  // Sell 100 @ 100.00
...
TEST_F(OrderBookTest, LargeOrderQuantities) {
    ...
    book->AddOrder(1, true, large_qty, 10000, 0, 0);
    book->AddOrder(2, false, large_qty, 10050, 0, 0);
...
TEST_F(OrderBookTest, ZeroQuantityOrder) {
    ...
    EXPECT_THROW(book->AddOrder(1, true, 0, 10000, 0, 0), std::invalid_argument);
...
TEST_F(OrderBookTest, ManyOrdersPerformance) {
    ...
    for (int i = 0; i < num_orders; ++i) {
        book->AddOrder(1, true, 100, 10000 - i, 0, 0);  // Decreasing prices
    }
    
    // Add many sell orders
    for (int i = 0; i < num_orders; ++i) {
        book->AddOrder(2, false, 100, 10100 + i, 0, 0);  // Increasing prices
    }
...
TEST_F(OrderBookTest, ModifyOrderQuantityDecrease) {
    // Add an order
    uint64_t order_id = book->AddOrder(1, true, 100, 10000, 0, 0);
    EXPECT_EQ(book->GetTotalBidVolume(), 100);
    
    // Decrease quantity
    book->ModifyOrder(order_id, 75, 10000, 0, 0);
...
TEST_F(OrderBookTest, ModifyOrderQuantityIncrease) {
    ...
    // Increase quantity
    book->ModifyOrder(order_id, 150, 10000, 0, 0);
    EXPECT_EQ(book->GetTotalBidVolume(), 150);
    EXPECT_EQ(book->GetBestBid(), 10000);
}
...
TEST_F(OrderBookTest, ModifyOrderPrice) {
    ...
    uint64_t order_id = book->AddOrder(1, true, 100, 9900, 0, 0);
    book->AddOrder(1, false, 100, 10100, 0, 0);

    EXPECT_EQ(book->GetBestBid(), 9900);
    EXPECT_EQ(book->GetBestAsk(), 10100);
    
    // Modify buy order to higher price (but still no match)
    book->ModifyOrder(order_id, 100, 10000, 0, 0);
    EXPECT_EQ(book->GetBestBid(), 10000);
    EXPECT_EQ(book->GetBestAsk(), 10100);
    EXPECT_EQ(book->GetTotalBidVolume(), 100);
    EXPECT_EQ(book->GetTotalAskVolume(), 100);
...
TEST_F(OrderBookTest, ModifyOrderPriceCausingMatch) {
    ...
    book->ModifyOrder(order_id, 100, 10200, 0, 0);
...
TEST_F(OrderBookTest, ModifyOrderErrorCases) {
    ...
    EXPECT_THROW(book->ModifyOrder(order_id, 0, 10000, 0, 0), std::invalid_argument);

    // Test modifying after cancellation
    book->CancelOrder(order_id);
    EXPECT_THROW(book->ModifyOrder(order_id, 100, 10000, 0, 0), std::runtime_error);
}
